"""
Trading Bot v5 COMPLET — Simulation Pure sur Marché Réel
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Toutes les fonctionnalités de v4 + nouvelles v5 :
  • Kelly Criterion avancé (taille optimale des trades)
  • Liquidations en temps réel (CoinGlass/Bybit funding)
  • Données on-chain (CoinGecko)
  • Détection arbitrage inter-exchanges (Bybit/Binance/Kraken)
  • Whale alerts (gros mouvements)
  • Scanner Polymarket (API publique)
  • Options Put/Call ratio (Deribit)
  • Micro-trading algo pur (sub-minute)
  • Memecoins DexScreener + traders AlxCooks/Murad/Ansem
  • Yahoo Finance (actions US, Forex, commodités)
  • Auto-ajustement SL/TP dynamique
  • Test stratégies en simulation
  • Résumé journalier automatique
  • Surveillance traders Twitter/YouTube
"""
...
MEMECOIN_SOLANA = [
    "BONKUSDT","WIFUSDT","POPCATUSDT","JUPUSDT","MEWUSDT",
]
MEMECOIN_ETH = [
    "SHIBUSDT","FLOKIUSDT","PEPEUSDT","DOGEUSDT",
]
...
2. CATHIE WOOD : Favoriser tokens innovants. RSI bas = entrée.
...
bot_state = {
    ...
    "nitter_idx": 0, "yt_idx": 0, "micro_count": 0,
    "meme_trader_idx": 0, "meme_cycle": 0,
}
...
def dynamic_position_size(confidence: int, market: str, symbol: str) -> float:
    ...
    try:
        ...
        if not closes.empty:
            ...
            if vol > 3:    vol_multiplier = 0.7
            elif vol < 1:  vol_multiplier = 1.2
    except Exception:
        pass
    size = base_kelly * conf_multiplier * market_mult * vol_multiplier
    return round(max(0.03, min(MAX_PCT_PER_TRADE, size)), 3)


# ═══════════════════════════════════════════════════════════════
#  NOUVELLES SOURCES DE DONNÉES v5
# ═══════════════════════════════════════════════════════════════
...
def interpret_liquidations(liq_data: dict) -> str:
    ...
    for sym, data in list(liq_data.items())[:3]:
        ...
        lines.append(f"  {emoji} {sym.replace('USDT','')}: {rate*100:.4f}% ({signal})")
    return "\n".join(lines)
...
def format_onchain(data: dict) -> str:
    ...
    return (f"🔗 MCap=${mcap_b:.0f}B ({chg:+.1f}%) | "
            f"BTC dom={data.get('btc_dominance',0)}%")
        ...
        for sym in ["BTCUSDT","ETHUSDT"]:
            ...
            if len(vols) >= 2:
                ...
                if ratio > 3:
                    ...
                    alerts.append({
                        "summary": f"Volume spike {coin} x{ratio:.1f}",
                        "symbol":  sym,
...
def get_polymarket_markets() -> list:
    ...
    try:
        r = requests.get(
            ...
            timeout=10, params={"active":"true","limit":20},
...
def format_polymarket(markets: list) -> str:
    ...
    best = markets[0]
    return (f"🎯 Poly: {best['question'][:50]} "
            ...
            for sym in ["BTCUSDT","ETHUSDT","SOLUSDT","BNBUSDT","XRPUSDT"]:
                ...
                if spread_pct > 0.15:
                    opportunities.append({
                        ...
                        "profit_est": round(spread_pct - 0.10, 3),
                    })
    except Exception as e:
        print(f"[ARB] {e}")
    try:
        r = requests.get(
            ...
            timeout=8)
        if r.status_code == 200:
            kraken_btc = float(r.json().get("result",{}).get("XXBTZUSD",{}).get("c",[0])[0])
...
def format_arbitrage(opps: list) -> str:
    ...
    for o in opps[:2]:
        ...
        lines.append(f"  💰 {coin}: {o['spread_pct']:.3f}% spread → ~{o['profit_est']:.3f}% net")
    return "\n".join(lines)
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS strategy_tests(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        strategy_name TEXT, params TEXT, trades INTEGER, win_rate REAL,
        total_pnl REAL, tested_date TEXT, active INTEGER DEFAULT 0
    )""")
    con.commit()
    con.close()
    ...
    if DATA_FILE.exists():
        try:
            ...
            for k,v in {"lessons":[],"patterns_to_avoid":[],"patterns_that_work":[],
                        "confidence_threshold":CONFIDENCE_BASE,"total_wins":0,"total_losses":0}.items():
        ...
        try:
            ...
            if abs(score) >= 2:
                opps.append({
                    ...
                    "market_type": market_name, "price": price,
                    "score":       score,
...
def _can_call_ai() -> bool:
    ...
    if now-_ai_calls["window_start"] > 3600:
        _ai_calls["count"]=0; _ai_calls["window_start"]=now
    if _ai_calls["count"] >= AI_CALLS_PER_HOUR_MAX: return False
    if now-_ai_calls["last_call"] < AI_CYCLE_MIN_INTERVAL: return False
    return True

def _register_ai_call():
    _ai_calls["count"]+=1; _ai_calls["last_call"]=time.time()


def ask_model_single(prompt: str, model: str=None) -> dict:
    if not _can_call_ai():
        return {"signal":"HOLD","confidence":0,"reason":"quota_horaire","risk":"HIGH"}
    if model is None: model=GROQ_FAST_MODEL
    for m in [model, GROQ_SECONDARY_MODEL]:
        try:
            ...
            if "decommissioned" in err or "400" in err: continue
...
# ═══════════════════════════════════════════════════════════════
#  ANALYSE COMPLÈTE AVEC TOUTES LES SOURCES
# ═══════════════════════════════════════════════════════════════
def analyze(opp: dict, fear_greed: str) -> dict:
    ...
    fg_context=""
    if fg_value<20: fg_context="⚠️ EXTREME FEAR → Opportunité rare"
    elif fg_value<35: fg_context="Fear élevé → opportunité potentielle"
...
def open_trade(analysis: dict, send_fn) -> dict | None:
    ...
    amount=sim["cash"]*kelly_pct; qty=amount/price
    sim["cash"]-=amount
...
def open_meme_trade(token: dict, score: int, source: str, send_fn) -> dict | None:
    ...
    trade={"id":len(sim["trades"])+1,"symbol":symbol,"market":"MEME","side":"LONG",
           ...
           "reason":f"Score {score}/10 | @{source} | {token.get('change_1h',0):+.1f}%/1h",
           "exit_reason":None,"time_in":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
           "time_out":None,"pnl":None,"pnl_pct":None,"duration_min":None,
           "patterns":["memecoin"],"leverage":1,"peak_price":price,
           "open_time":time.time(),"chain":chain,
           "token_address":token.get("address",""),"kelly_pct":MEME_MAX_PCT}
    pos_key=f"MEME_{symbol}_{trade['id']}"
    sim["trades"].append(trade); sim["positions"][pos_key]={**trade,"pos_key":pos_key}
    db_save_trade(trade); bot_state["trades_today"]+=1
    sl=price*(1-MEME_SL_PCT); tp=price*(1+MEME_TP_PCT)
    send_fn(
        f"🐸 MEME ${symbol} ({chain.upper()})\n"
        f"  ${price:.8f} | Liq:${token.get('liquidity',0)/1000:.0f}k\n"
        f"  +{token.get('change_1h',0):.1f}%/1h | Score:{score}/10\n"
        f"  Mise:${amount:.2f} | SL:-{MEME_SL_PCT*100:.0f}% TP:+{MEME_TP_PCT*100:.0f}%"
    )
    return trade
...
def monitor_meme_positions(send_fn):
    ...
    for pos_key,pos in list(sim["positions"].items()):
        ...
        entry=pos["price_in"]; elapsed=now-pos.get("open_time",now)
        try:
        ...
        elif change>0.05 and trailing>=MEME_TRAILING_PCT: reason=f"📐 MEME TRAIL"
        elif elapsed>=MEME_MAX_DURATION: reason=f"⏱ TIMEOUT"
        if reason:
            pnl=change*pos["amount_usd"]; sim["cash"]+=pos["amount_usd"]+pnl
            trade=next((t for t in reversed(sim["trades"]) if t["id"]==pos["id"]),None)
            if trade:
...
def run_meme_cycle(send_fn):
    ...
    for data in trending[:3]:
        ...
        if data.get("change_5m",0)>5 and data.get("volume_1h",0)>5000:
            ...
            open_meme_trade(data,score,"DexScreener",send_fn)
    # Memecoins Bybit classiques
    prices=get_prices_batch()
        ...
        if score>=6:
            ...
            if meme_count<2:
                token_data={"symbol":sym.replace("USDT",""),"price":price,
                            ...
                            "address":""}
                open_meme_trade(token_data,score,"Bybit",send_fn)
...
def auto_adjust_sl_tp():
    ...
    if sl_hits/len(recent)>0.5 and STOP_LOSS_PCT<0.04:
        ...
        print(f"[AUTO-SL] {STOP_LOSS_PCT*100:.1f}%")
...
Kelly moyen:{avg_kelly}% SL={STOP_LOSS_PCT*100:.1f}% TP={TAKE_PROFIT_PCT*100:.1f}%
JSON:{{"rules":["règle1","règle2","règle3"],"sl_pct":2.5,"tp_pct":4.0,"insight":"insight"}}"""
        r=ask_model_single(prompt); rules=r.get("rules",[])
        for rule in rules:
            try:
                con=sqlite3.connect(DB_FILE)
...
def test_strategy_variation(send_fn):
    ...
    for strat in strategies:
        simulated_wins=sum(1 for t in recent if t.get("pnl_pct",0)>=strat["tp"]*100)
    ...
    if best_strat and best_wr>current_wr+5:
        ...
        send_fn(f"🧬 ÉVOLUTION: {best_strat['name']}\nWR simulé:{best_wr:.0f}% vs {current_wr:.0f}%")
# ═══════════════════════════════════════════════════════════════
#  BOUCLE PRINCIPALE
# ═══════════════════════════════════════════════════════════════
def trading_loop(send_fn):
    ...
    send_fn(
        f"🚀 BOT v5 COMPLET DÉMARRÉ\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"💰 Capital    : ${CAPITAL_INITIAL:,.2f} (virtuel)\n"
        f"📐 Kelly init : {kelly_init*100:.1f}% par trade\n"
        f"⚡ Micro      : -{MICRO_SL_PCT*100:.1f}%/+{MICRO_TP_PCT*100:.1f}%\n"
        f"🔍 Scalp      : -{STOP_LOSS_PCT*100:.1f}%/+{TAKE_PROFIT_PCT*100:.1f}%\n"
        f"🎯 Polymarket : scanner actif\n"
        f"⚡ Arbitrage  : Bybit/Binance/Kraken\n"
        f"💥 Liquidations: actif\n"
        f"🔗 On-chain   : CoinGecko actif\n"
        f"📊 Options    : Deribit actif\n"
...
RSI:{o['ind'].get('rsi','?')} mom5:{o['ind'].get('mom5','?')}% score:{o['score']:+d}
{fear_greed}
...
JSON:{{"signal":"{direction}/HOLD","confidence":0-100,"reason":"raison","risk":"LOW/MEDIUM/HIGH","market":"FUTURES"}}"""
                        ...
                        result.update({"symbol":symbol,"price":price,"patterns":[],
                                       "confluence":conf,"ob":ob,"ind":ind5m,
        ...
        if now-bot_state["last_status"]>=CYCLE_STATUS:
            try:
                ...
                options=get_options_data(); onchain=get_onchain_data()

                fg_val=50
                try: fg_val=int(get_fear_greed().split(":")[1].split("/")[0].strip())
                except Exception: pass
                if fg_val<20:   trader_tip="💡 Saylor/Buffett : Fear extrême = opportunité rare"
                elif fg_val<35: trader_tip="💡 Buffett : Sois avide quand les autres ont peur"
                elif fg_val>75: trader_tip="💡 Paul Tudor Jones : Marché euphorique → protège le capital"
                elif stats["win_rate"]>60: trader_tip="💡 Livermore : Tu es en forme, laisse courir"
                else: trader_tip="💡 Cathie Wood : Focus sur les tokens à fort momentum"
...
def daily_summary(send_fn):
    while True:
        ...
        try:
            ...
            sym_s=db_symbol_stats()
            best3="\n".join(f"  🏅 {s['s']}: WR {s['wr']:.0f}% ({s['n']} trades)" for s in sym_s[:3]) or "  Aucun"
...
def generate_dashboard() -> str:
    ...
    kelly=kelly_criterion(); wr_db=db_win_rate(30)
    sym_s=db_symbol_stats()
    last=bot_state.get("last_heartbeat")
    ...
    for pk,pos in sim["positions"].items():
        ...
        pos_html+=(f"<tr><td>{pos['symbol'].replace('USDT','')}</td>"
    ...
    for m in poly_mkts[:3]:
        ...
        poly_html+=(f"<tr><td>{m['question'][:50]}</td>"
                    f"<td>{m['yes_price']:.2f}</td><td>{m['no_price']:.2f}</td>"
...
.cmds{{background:#161b22;border-radius:8px;padding:10px;margin-bottom:10px;
       font-size:.78em;line-height:1.8}}
code{{color:#58a6ff}}
</style>
<meta http-equiv="refresh" content="20">
</head><body>
<h1>🤖 Trading Bot v5 — Kelly + Arbitrage + Polymarket + Memecoins</h1>
...
<div class="grid">
  ...
  <div class="card"><div class="label">Options P/C | Sent.</div>
...
<code>/positions</code> <code>/lecons</code> <code>/scan</code>
<code>/arbitrage</code> <code>/polymarket</code> <code>/kelly</code>
<code>/marches</code> <code>/memes</code> <code>/signaux</code>
<code>/regles</code> <code>/stats</code> <code>/apprendre</code>
...
<h2>Arbitrage Inter-Exchanges</h2>
<table><thead><tr>
...
<h2>Polymarket — Inefficacités de Prix</h2>
<table><thead><tr>
  <th>Question</th><th>YES</th><th>NO</th><th>Ineff.</th>
</tr></thead><tbody>
...
def make_send(chat_id: str):
    def send(msg: str):
        ...
        f=asyncio.run_coroutine_threadsafe(_app.bot.send_message(chat_id=chat_id,text=msg),_main_loop)
...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📊 P/C ratio : {options.get('put_call_ratio','N/A')} ({options.get('sentiment','N/A')})\n"
        f"📚 Leçons   : {len(memory['lessons'])}")
...
async def cmd_arbitrage(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not opps: await update.message.reply_text("Aucune opportunité détectée."); return
    lines=["⚡ ARBITRAGE\n━━━━━━━━━━━━━"]
    for o in opps:
...
async def cmd_polymarket(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not markets: await update.message.reply_text("Aucune inefficacité détectée."); return
    lines=["🎯 POLYMARKET\n━━━━━━━━━━━━━"]
    for m in markets:
...
async def cmd_positions(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for pk,pos in sim["positions"].items():
        p=prices.get(pos["symbol"],pos["price_in"])
        chg=(p-pos["price_in"])/pos["price_in"]*100*pos.get("leverage",1)
        lines.append(f"{'📈' if chg>0 else '📉'} {pos['symbol'].replace('USDT','')} {pos['market']} {pos['side']}\n"
                     f"  ${pos['price_in']:.4f}→${p:.4f} ({chg:+.2f}%) Kelly:{pos.get('kelly_pct',0)*100:.1f}%")
    await update.message.reply_text("\n".join(lines))
...
async def cmd_memes(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        trending=dex_get_trending_solana(); lines=["🐸 MEMECOINS\n━━━━━━━━━━━━━"]
        if trending:
...
async def cmd_regles(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        con=sqlite3.connect(DB_FILE)
        rows=con.execute("""SELECT rule,win_rate,sample_size,created_date
            FROM trading_rules WHERE active=1 ORDER BY win_rate DESC LIMIT 10""").fetchall()
        con.close()
        if not rows:
            await update.message.reply_text(
                f"🧠 Aucune règle encore.\nTrades:{len([t for t in sim['trades'] if t.get('pnl')])}/10"); return
...
async def cmd_stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Top coins:\n{sym_lines}\n━━━━━━━━━━━━━\n"
        f"📚 Leçons:{len(memory['lessons'])} | ✅{len(memory['patterns_that_work'])} | ❌{len(memory['patterns_to_avoid'])}")
...
async def cmd_apprendre(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Seuil:{LEARN_MODE_CONF_MIN}% | Taille:{LEARN_MODE_MAX_PCT*100:.0f}%\n"
...
async def run_telegram():
    ...
    print("Bot v5 prêt — /start pour lancer")
    try:
...
def auto_start():
    ...
    bot_state.update({"running":True,"trades_today":0,"cycle_count":0,
                      ...
                      "last_scalp":0,"last_deep":0,"last_status":0,"last_meme":0})
    kelly=kelly_criterion()
    send(f"🔄 Bot v5 redémarré automatiquement\nKelly:{kelly*100:.1f}% | /stop pour arrêter")
    threading.Thread(target=trading_loop,args=(send,),daemon=True).start()
    threading.Thread(target=watchdog,args=(send,),daemon=True).start()
    threading.Thread(target=daily_summary,args=(send,),daemon=True).start()
