...
def learn_from_trade(trade):
    ...
    try:
        ...
        verdict = "TRADE PERDANT. Explique pourquoi et comment eviter ca." if trade["pnl"] < 0 \
                  else "TRADE GAGNANT. Explique pourquoi ca a marche."
...
Reponds en JSON strict (pas de backticks):
{{"lecon": "lecon courte", "pattern": "pattern identifie", "action_future": "quoi faire differemment", "type": "erreur ou succes"}}"""
        ...
        # Garder les 50 dernieres lecons
...
Reponds UNIQUEMENT en JSON strict (pas de backticks):
...
Ne prends position que si confidence >= 72 et risk = LOW."""
    ...
    try:
        ...
        return {
            "signal": "HOLD", "confidence": 0, "reason": "Erreur",
            "risk": "HIGH", "sentiment": "neutral", "key_signal": "",
        }, price


# ─────────────────────────────────────────────
#  BOUCLE PRINCIPALE DU BOT (thread séparé)
# ─────────────────────────────────────────────
def bot_loop(send_fn):
    """
    Tourne dans un thread daemon. send_fn(msg) est une coroutine async
    qu'on appelle via asyncio.run() dans ce thread.
    """
    while bot_state["running"]:
        try:
            ...
            message = (
                ...
                f"━━━━━━━━━━━━━\n"
                f"Trades: {stats['total']} | Win: {stats['win_rate']}%\n"
                f"Meilleur: +${stats['best']} | Pire: ${stats['worst']}\n"
                f"Lecons: {len(memory['lessons'])}"
            )
...
def generate_dashboard():
    ...
    for t in reversed(portfolio["trades"][-20:]):
        ...
        price_out = f"${t['price_out']:,.2f}" if t["price_out"] else "-"
        trades_html += (
            f"<tr>"
            f"<td>{t['id']}</td><td>{t['type']}</td>"
            f"<td>${t['price_in']:,.2f}</td><td>{price_out}</td>"
            f"<td>{pnl_str}</td><td>{t['confidence']}%</td>"
            f"<td>{t['time_in']}</td><td>{t['reason'][:40]}</td>"
            f"</tr>"
        )
    ...
    for l in reversed(memory["lessons"][-10:]):
        ...
        lessons_html += (
            f'<tr>'
            f'<td style="color:{color}">{emoji}</td>'
            f'<td style="color:{color}">${pnl_val:+.2f}</td>'
            f"<td>{l['lecon'][:60]}</td>"
            f"<td>{l['action_future'][:60]}</td>"
            f"<td>{l['date']}</td>"
            f"</tr>"
        )
...
# ─────────────────────────────────────────────
#  SERVEUR HTTP (health check + dashboard)
# ─────────────────────────────────────────────
...
async def cmd_analyse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        analysis, price = analyze_market()
        await update.message.reply_text(
            f"Analyse instantanee\n"
            f"BTC: ${price:,.2f}\n"
            f"Sentiment: {analysis.get('sentiment', 'neutral')}\n"
            f"Signal: {analysis.get('signal', 'HOLD')} ({analysis.get('confidence', 0)}%)\n"
            f"Risque: {analysis.get('risk', 'HIGH')}\n"
            f"Signal cle: {analysis.get('key_signal', '')[:80]}\n"
            f"Raison: {analysis.get('reason', '')}"
        )
    except Exception as e:
...
async def cmd_portfolio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats = get_stats()
    await update.message.reply_text(
        f"Portefeuille Virtuel\n"
        f"Capital initial: $1000.00\n"
        f"Valeur actuelle: ${pv:,.2f}\n"
        f"PnL Total: ${pnl:+.2f}\n"
        f"Cash: ${portfolio['cash']:,.2f}\n"
        f"BTC: {portfolio['btc']:.6f}\n"
        f"Position: {'EN POSITION' if portfolio['btc'] > 0.00001 else 'CASH'}\n"
        f"Trades fermes: {stats['total']}\n"
        f"Wins: {stats['wins']} | Losses: {stats['losses']}\n"
        f"Win Rate: {stats['win_rate']}%\n"
        f"Meilleur: +${stats['best']}\n"
        f"Pire: ${stats['worst']}"
    )
...
async def cmd_reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not _authorized(update):
        return
    bot_state["running"] = False
    portfolio.update({"cash": 1000.0, "btc": 0.0, "trades": [], "position": None})
    memory.update({
...
async def run_telegram():
    # Connection pool élargi + timeouts explicites pour éviter les pool timeouts
    ...
    # drop_pending_updates=True : ignore les updates accumulées pendant les crashs/redémarrages
    # Évite aussi les conflits quand Koyeb lance plusieurs instances en séquence
    ...
    try:
        ...
        # Arrêt propre : stoppe le polling avant de fermer
...
if __name__ == "__main__":
    ...
    # Serveur HTTP dans un thread daemon (health checks Koyeb)
